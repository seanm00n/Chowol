...
public class TileManager : MonoBehaviour {
    ...
    public void DistortionBreak() {
        ...
        Debug.Log("Distortion Tile activated");
    }
    ...
    private void CreateSpec() {
        ...
        if(_tiles[_speciaTileIndex].GetTileType() != ETileType.brok) { // 안 부서졌을 시 복구
        ...
        switch(selectedCard._type) {
            ...
            case ECardType.explosion: // x모양 전부 //수정
                condition = (newTarget) =>
                    ...
                    (Math.Abs((target / 8) - (newTarget / 8)) == Math.Abs((target % 8) - (newTarget % 8)))); // 대각선 체크
            ...
            case ECardType.thunderbolt: // 작은 십자
                condition = (newTarget) =>
                    (newTarget >= 0 && newTarget < _tiles.Length &&
                    (target / 8 == newTarget / 8 || target % 8 == newTarget % 8)); // 같은 행 또는 같은 열
            ...
            case ECardType.shockwave: // 9*9 네모
                condition = (newTarget) =>
                    (newTarget >= 0 && newTarget < _tiles.Length); // 모든 타일 포함
                AddTileIfValid( tmp, target, new[] { 1, 9, 8, 7, -1, -9, -8, -7 }, condition);
                break;
            case ECardType.earthquake: // 가로 전부
                condition = (newTarget) =>
                    (newTarget >= 0 && newTarget < _tiles.Length && (target / 8 == newTarget / 8)); // 같은 행
            ...
            case ECardType.resonance: // 상하좌우 2개씩
                condition = (newTarget) =>
                    (newTarget >= 0 && newTarget < _tiles.Length &&
                    (Math.Abs(newTarget - target) == 1 || Math.Abs(newTarget - target) == 8)); // 인접한 상하좌우 체크
